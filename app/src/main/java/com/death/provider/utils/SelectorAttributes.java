...
public class SelectorAttributes {
    ...
    {
        return new String[]{BASE_PAYTM+query, BASE_AMAZON+query, BASE_FLIPKART+query};
    }

    public static String getStoreName(String link)
    {
        if(link.contains(BASE_AMAZON))
        {
            return "AMAZON";
        }
        else if(link.contains(BASE_FLIPKART))
        {
            return  "FLIPKART";
        }
        else if(link.contains(BASE_PAYTM)){
            return  "PAYTM";
        }
        return "DEFAULT";
    }
